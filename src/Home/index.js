...
export class Home extends Component {
    ...
    __searchProduct = (e) => {
        const filteredProducts = this.state.products.filter(
            product => product.Ingredient && product.Ingredient.toLowerCase().match(e.target.value.toLowerCase()),
        );
        this.setState({ search: true, filteredProducts: filteredProducts })
    }
    render() {
        return (
            <React.Fragment>
                <div className='header-fixed'>
                    <div className='d-flex flex-row justify-content-start align-items-start' style={{ borderBottom: '2px solid black' }}>
                        <div className='flex-grow-1 w-100  px-3 py-3'>
                            <span className='fs-3 fw-bolder font-color'>Search</span>
                        </div>
                    </div>
                    <div className='px-3 py-3'>
                        <div>
                            <div className="input-group   b-r-10" style={{ border: 'none', outline: 'none' }}>
                                ...
                                <input type="text" className="form-control font-color search-input py-3" onChange={(e) => this.__searchProduct(e)} placeholder="Food name" aria-label="Food Name" aria-describedby="Food Name" />
                            </div>
                        </div>
                    </div>
                </div>
                <div className='p-3' style={{ backgroundColor: '#f7f7f7' }}>
                    <div className=''>
                        <span className='fs-5 fw-bold font-color'>Foods</span>
                    </div>
                    <div style={{ display: 'grid', gap: '20px', gridTemplateColumns: 'repeat(2,1fr)', justifyItems: 'center' }}>
                        {this.state.products && this.state.products.length > 0 && !this.state.search ? this.state.products.map((product, index) => (
                            <React.Fragment>
                                <Products product={product} />
                            </React.Fragment>
                        )) : this.state.filteredProducts && this.state.filteredProducts.length > 0 && this.state.filteredProducts.map((product, index) => (
                            <React.Fragment>
                                <React.Fragment>
                                    <Products product={product} />
                                </React.Fragment>
